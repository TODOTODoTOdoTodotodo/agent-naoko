...
## 📅 2026-01-20: 리뷰/적용 안정화 및 품질 개선 (v0.3.0)

### 1. 엔트리포인트 참조 모드 강화
- 기존 프로젝트 모드에서 엔트리포인트 파일은 **참조 전용**으로 유지.
- import 네임스페이스 일관성(예: javax/jakarta 혼용 금지) 가이드 추가.

### 2. 리뷰 입력 신뢰성 개선
- 리뷰 대상 파일을 `git diff` + untracked 기준으로 수집.
- 상위 Git 루트 prefix 정규화로 서브디렉토리 프로젝트도 정상 인식.

### 3. 리뷰 루프 실패 조건 명확화
- 생성 코드 누락/수정 무효 시 즉시 FAILED 처리.
- 불필요한 사용자 질문 프롬프트 제거.

### 4. 산출물 확장
- 완료 시 `artifacts/diagrams.md`에 Mermaid 클래스 다이어그램과 플로우차트 생성.
